...
def movie_to_ostruct(movies) # => Array with OpenStruct
  movies.map do |movie|
    OpenStruct.new(link: movie[0],
                   ...
                   origin: movie[3],
                   release_date: Date.parse(normalizing_dates(movie[4])),
                   genre: movie[5],
...
def sorted_by_origin(movies, country)
  sorted_by_year(movies).reject { |movie| movie[:origin].include?(country) }
end

def sorted_by_month(movies)
  sorted_by_year(movies).sort_by { |date| date[:release_date].mon }
end
...
sorted_by_genre(movies_struct, 'Comedy').first(10).each { |movie| render_hash_movies(movie) }

puts "Movies filmed outside USA: \n"
sorted_by_origin(movies_struct, 'USA').each { |movie| render_hash_movies(movie) }
